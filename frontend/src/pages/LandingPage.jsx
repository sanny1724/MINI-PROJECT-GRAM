...
export default function LandingPage() {
  ...
  const [modalSelectedVillage, setModalSelectedVillage] = useState(null);
  ...
  const handleModalLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role !== selectedRole) {
        const dispRole = selectedRole === 'panchayat' ? 'Village Secretary' :
                         selectedRole === 'tahsildar' ? 'Mandal Officer' :
                         selectedRole === 'collector' ? 'District Officer' : 'State Officer';
        toast.error(`Access Denied: Your profile does not match the ${dispRole} role.`);
        setLoginLoading(false);
        return;
      }

      // 2. LGD Administrative Scope Enforcement
      let isAuthorized = false;
      const userLgd = user.lgd_code;

      if (selectedRole === 'state') {
        isAuthorized = true;
      } else if (selectedRole === 'collector') {
        if (Number(userLgd) === Number(modalSelectedDistrict.code)) {
          isAuthorized = true;
        } else {
          toast.error(`Access Denied: You are not authorized to manage ${modalSelectedDistrict.name} District (your scope: LGD ${userLgd}).`);
        }
      } else if (selectedRole === 'tahsildar') {
        if (Number(userLgd) === Number(modalSelectedMandal.id)) {
          isAuthorized = true;
        } else {
          toast.error(`Access Denied: You are not authorized to manage ${modalSelectedMandal.name} Mandal (your scope: LGD ${userLgd}).`);
        }
      } else if (selectedRole === 'panchayat') {
        if (Number(userLgd) === Number(modalSelectedVillage.id)) {
          isAuthorized = true;
        } else {
          toast.error(`Access Denied: You are not authorized to manage ${modalSelectedVillage.name} Village (your scope: LGD ${userLgd}).`);
        }
      }
      ...
      toast.success(`Welcome back, Officer ${user.full_name || user.email}!`);
  ...
  const openGeneralWizard = (purpose) => {
  ...
  return (
    <div className="min-h-screen bg-[#F5F1E7] text-[#17352A] font-sans selection:bg-[#B87932] selection:text-white overflow-x-hidden">
      ...
      <section className="py-12 bg-[#F5F1E7] border-b border-[#17352A]/12 relative">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-5 gap-8">
          
          <div className="flex flex-col items-center text-center">
            <span className="counter-text font-heading text-4xl md:text-5xl text-[#B87932] font-bold">{stats.districts}</span>
            <span className="text-[11px] text-[#5F7668] font-mono tracking-wider uppercase mt-2">Districts</span>
          </div>

          <div className="flex flex-col items-center text-center">
            <span className="counter-text font-heading text-4xl md:text-5xl text-[#B87932] font-bold">{stats.mandals}</span>
            <span className="text-[11px] text-[#5F7668] font-mono tracking-wider uppercase mt-2">Mandals</span>
          </div>

          <div className="flex flex-col items-center text-center">
            <span className="counter-text font-heading text-4xl md:text-5xl text-[#B87932] font-bold">{stats.villages.toLocaleString()}</span>
            <span className="text-[11px] text-[#5F7668] font-mono tracking-wider uppercase mt-2">Villages</span>
          </div>

          <div className="flex flex-col items-center text-center">
            <span className="counter-text font-heading text-4xl md:text-5xl text-[#B87932] font-bold">{stats.citizens}M+</span>
            <span className="text-[11px] text-[#5F7668] font-mono tracking-wider uppercase mt-2">Citizens Enrolled</span>
          </div>

          <div className="flex flex-col items-center text-center col-span-2 md:col-span-1">
            <div className="flex items-center gap-2 justify-center">
              <span className="w-2.5 h-2.5 bg-green-500 rounded-full animate-ping" />
              <span className="font-heading text-2xl md:text-3xl text-[#17352A] font-bold">24×7</span>
            </div>
            <span className="text-[11px] text-[#5F7668] font-mono tracking-wider uppercase mt-2">AI Governance Monitoring</span>
          </div>
      ...
      <section className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12"><div className="max-w-7xl mx-auto">
        <div className="text-center flex flex-col items-center gap-4 mb-16">
          <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Governance Pipeline</span>
          <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">What is GRAM?</h2>
          <div className="w-16 h-1 bg-[#B87932]" />
        </div>

        {/* 3D Animated Governance Pipeline */}
        <GovernancePipeline3D />
      </div>
      </section>

      {/* SECTION 7 — EVERY VILLAGE HAS A STORY */}
      <section id="story" className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12"><div className="max-w-7xl mx-auto">
        <div className="text-center flex flex-col items-center gap-4 mb-16">
          <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Village-First Design</span>
          <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">Every Village Has a Story</h2>
          <div className="w-16 h-1 bg-[#B87932]" />
          <p className="text-sm text-[#5F7668] max-w-2xl mt-2 font-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300">
            ...
            <h3 className="font-heading text-lg font-bold text-[#17352A]">Water Supply</h3>
            <p className="text-xs text-[#5F7668] leading-relaxed font-light">
              Audits Mission Bhagiratha pipeline pressures, chlorination levels, overhead tanks, and community borewell status.
            </p>
            <div className="mt-auto pt-4 border-t border-[#17352A]/5 text-[9px] font-mono text-[#B87932] uppercase tracking-wider">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300">
            ...
            <h3 className="font-heading text-lg font-bold text-[#17352A]">Schooling</h3>
            <p className="text-xs text-[#5F7668] leading-relaxed font-light">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300">
            ...
            <h3 className="font-heading text-lg font-bold text-[#17352A]">Health PHC</h3>
            <p className="text-xs text-[#5F7668] leading-relaxed font-light">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300">
            ...
            <h3 className="font-heading text-lg font-bold text-[#17352A]">Agriculture</h3>
            <p className="text-xs text-[#5F7668] leading-relaxed font-light">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300">
            ...
            <h3 className="font-heading text-lg font-bold text-[#17352A]">Governance</h3>
            <p className="text-xs text-[#5F7668] leading-relaxed font-light">
      ...
      <section className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12 relative overflow-hidden">
        
        <div className="max-w-7xl mx-auto">
          <div className="text-center flex flex-col items-center gap-4 mb-16">
            <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Governance Transition</span>
            <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">From Data to Action</h2>
            <div className="w-16 h-1 bg-[#B87932]" />
            <p className="text-sm text-[#5F7668] max-w-xl mt-2 font-light">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 relative">
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
              <div className="w-16 h-16 rounded-full bg-white border border-[#17352A]/10 hover:border-[#B87932] flex items-center justify-center font-heading text-xl font-bold text-[#B87932] shadow-sm hover:shadow-md transition-all duration-300">
                1
              </div>
              <h4 className="font-heading text-base font-bold text-[#17352A] uppercase tracking-wider">Ground Data</h4>
              <p className="text-[11px] text-[#5F7668] leading-relaxed max-w-[180px] font-light">
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
              ...
              <h4 className="font-heading text-base font-bold text-[#17352A] uppercase tracking-wider">AI Risk Engine</h4>
              <p className="text-[11px] text-[#5F7668] leading-relaxed max-w-[180px] font-light">
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
              ...
              <h4 className="font-heading text-base font-bold text-[#17352A] uppercase tracking-wider">Village Insight</h4>
              <p className="text-[11px] text-[#5F7668] leading-relaxed max-w-[180px] font-light">
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
              ...
              <h4 className="font-heading text-base font-bold text-[#17352A] uppercase tracking-wider">Official Action</h4>
              <p className="text-[11px] text-[#5F7668] leading-relaxed max-w-[180px] font-light">
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
              ...
              <h4 className="font-heading text-base font-bold text-[#17352A] uppercase tracking-wider">Public Trust</h4>
              <p className="text-[11px] text-[#5F7668] leading-relaxed max-w-[180px] font-light">
      ...
      <section className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12"><div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 flex flex-col gap-5">
            <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Citizen Empowerment</span>
            <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">Your Village.<br />Your Information.</h2>
            <div className="w-16 h-1 bg-[#B87932]" />
            <p className="text-sm text-[#5F7668] leading-relaxed font-light">
            ...
            <div className="flex flex-col gap-4 mt-2">
              <div className="flex items-start gap-4">
                <span className="w-6 h-6 rounded-lg bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-mono text-[10px] font-bold text-[#B87932] shrink-0 mt-0.5">01</span>
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#17352A]">Select District</h4>
                  <p className="text-[11px] text-[#5F7668]">Navigate the State Map or directory to find your district.</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <span className="w-6 h-6 rounded-lg bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-mono text-[10px] font-bold text-[#B87932] shrink-0 mt-0.5">02</span>
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#17352A]">Select Mandal</h4>
                  <p className="text-[11px] text-[#5F7668]">Drill down into Mandal coordinates to list local villages.</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <span className="w-6 h-6 rounded-lg bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-mono text-[10px] font-bold text-[#B87932] shrink-0 mt-0.5">03</span>
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#17352A]">Inspect Scorecard</h4>
                  <p className="text-[11px] text-[#5F7668]">Inspect live scores, welfare budgets, and submit local grievances.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 bg-white border border-[#17352A]/10 rounded-3xl p-8 relative overflow-hidden shadow-md">
            <div className="flex justify-between items-start border-b border-[#17352A]/10 pb-6 mb-6">
              <div>
                <h3 className="font-heading font-extrabold text-2xl text-[#17352A]">Ankapur Village</h3>
                <span className="text-xs font-mono text-[#5F7668]">LGD Code: 569005 • Nizamabad District</span>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-xs font-mono text-[#B87932] uppercase font-bold tracking-wider">Overall Score</span>
                <span className="text-3xl font-heading font-black text-[#17352A] font-mono-num">89<span className="text-sm font-light text-[#5F7668]">/100</span></span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between bg-green-500/10 border border-green-500/20 px-4 py-2.5 rounded-xl mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-green-700 font-bold uppercase tracking-wider">AI Risk Level: LOW</span>
              </div>
              <span className="text-[10px] text-[#5F7668] font-mono">Last Audited: Just Now</span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <div className="bg-[#F5F1E7] border border-[#17352A]/5 p-3 rounded-xl flex flex-col items-center text-center">
                <Droplet className="w-5 h-5 text-blue-500 mb-1" />
                <span className="text-[9px] font-mono uppercase text-[#5F7668]">Water</span>
                <span className="font-heading font-bold text-sm text-[#17352A] mt-1">94</span>
                <span className="text-[8px] text-green-700 font-bold uppercase mt-1">LOW Risk</span>
              </div>
              <div className="bg-[#F5F1E7] border border-[#17352A]/5 p-3 rounded-xl flex flex-col items-center text-center">
                <GraduationCap className="w-5 h-5 text-indigo-500 mb-1" />
                <span className="text-[9px] font-mono uppercase text-[#5F7668]">Schooling</span>
                <span className="font-heading font-bold text-sm text-[#17352A] mt-1">88</span>
                <span className="text-[8px] text-green-700 font-bold uppercase mt-1">LOW Risk</span>
              </div>
              <div className="bg-[#F5F1E7] border border-[#17352A]/5 p-3 rounded-xl flex flex-col items-center text-center">
                <Activity className="w-5 h-5 text-red-500 mb-1" />
                <span className="text-[9px] font-mono uppercase text-[#5F7668]">Health</span>
                <span className="font-heading font-bold text-sm text-[#17352A] mt-1">81</span>
                <span className="text-[8px] text-yellow-700 font-bold uppercase mt-1">MED Risk</span>
              </div>
              <div className="bg-[#F5F1E7] border border-[#17352A]/5 p-3 rounded-xl flex flex-col items-center text-center">
                <Sprout className="w-5 h-5 text-green-500 mb-1" />
                <span className="text-[9px] font-mono uppercase text-[#5F7668]">Agri</span>
                <span className="font-heading font-bold text-sm text-[#17352A] mt-1">92</span>
                <span className="text-[8px] text-green-700 font-bold uppercase mt-1">LOW Risk</span>
              </div>
              <div className="bg-[#F5F1E7] border border-[#17352A]/5 p-3 rounded-xl flex flex-col items-center text-center col-span-2 md:col-span-1">
                <ShieldAlert className="w-5 h-5 text-yellow-600 mb-1" />
                <span className="text-[9px] font-mono uppercase text-[#5F7668]">Govern</span>
                <span className="font-heading font-bold text-sm text--[#17352A] mt-1">90</span>
                <span className="text-[8px] text-green-700 font-bold uppercase mt-1">LOW Risk</span>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          
          {/* Visual Scope Hierarchy Diagram */}
          <div className="lg:col-span-7 grid grid-cols-1 gap-4">
            {/* Village Scope Card */}
            <div className="bg-white border border-[#17352A]/10 p-5 rounded-2xl flex items-center gap-5 shadow-sm hover:border-[#B87932]/30 transition-all duration-300">
              <div className="w-12 h-12 rounded-xl bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-heading text-lg font-bold text-[#B87932]">01</div>
              <div className="flex flex-col">
                <h4 className="font-heading text-base font-bold text-[#17352A]">Village Officer (Sarpanch / Secretary)</h4>
                <p className="text-xs text-[#5F7668] leading-relaxed font-light mt-0.5">
            ...
            {/* Mandal Scope Card */}
            <div className="bg-white border border-[#17352A]/10 p-5 rounded-2xl flex items-center gap-5 shadow-sm hover:border-[#B87932]/30 transition-all duration-300">
              <div className="w-12 h-12 rounded-xl bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-heading text-lg font-bold text-[#B87932]">02</div>
              <div className="flex flex-col">
                <h4 className="font-heading text-base font-bold text-[#17352A]">Mandal Administration (Tahsildar / MPDO)</h4>
                <p className="text-xs text-[#5F7668] leading-relaxed font-light mt-0.5">
            ...
            {/* District Scope Card */}
            <div className="bg-white border border-[#17352A]/10 p-5 rounded-2xl flex items-center gap-5 shadow-sm hover:border-[#B87932]/30 transition-all duration-300">
              <div className="w-12 h-12 rounded-xl bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-heading text-lg font-bold text-[#B87932]">03</div>
              <div className="flex flex-col">
                <h4 className="font-heading text-base font-bold text-[#17352A]">District Collector</h4>
                <p className="text-xs text-[#5F7668] leading-relaxed font-light mt-0.5">
            ...
            {/* State Scope Card */}
            <div className="bg-white border border-[#B87932]/35 border-dashed p-5 rounded-2xl flex items-center gap-5 shadow-sm">
              <div className="w-12 h-12 rounded-xl bg-[#B87932]/10 border border-[#B87932]/25 flex items-center justify-center font-heading text-lg font-bold text-[#B87932]">04</div>
              <div className="flex flex-col">
                <h4 className="font-heading text-base font-bold text-[#17352A]">State Governance (Ministers / Directors)</h4>
                <p className="text-xs text-[#5F7668] leading-relaxed font-light mt-0.5">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Scope-Based Access</span>
            <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">From Village<br />to District</h2>
            <div className="w-16 h-1 bg-[#B87932]" />
            <p className="text-sm text-[#5F7668] leading-relaxed font-light">
            ...
            <div className="mt-2">
              <button 
                onClick={() => navigate('/login')}
                className="bg-[#B87932] hover:bg-[#B87932]/90 text-white font-bold text-xs font-mono px-6 py-3.5 rounded-xl transition-all shadow-md"
              >
      ...
      <section className="py-24 px-6 bg-[#F5F1E7] border-b border-[#17352A]/12 text-[#17352A]"><div className="max-w-7xl mx-auto">
        <div className="text-center flex flex-col items-center gap-4 mb-16">
          <span className="text-xs font-mono font-bold tracking-widest text-[#B87932] uppercase">Access Gates</span>
          <h2 className="font-heading text-4xl md:text-5xl text-[#17352A] font-bold leading-tight">Officer & Admin Portals</h2>
          <div className="w-16 h-1 bg-[#B87932]" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300 justify-between h-[250px]">
            <div className="flex flex-col gap-2">
              <span className="text-[#B87932] font-mono font-bold text-xs uppercase tracking-wider">Citizen</span>
              <h3 className="font-heading text-xl font-bold text-[#17352A]">View Village</h3>
              <p className="text-xs text-[#5F7668] leading-relaxed font-light">
            ...
            <button 
              ...
              className="text-[#B87932] font-bold text-xs font-mono flex items-center gap-1 group hover:underline"
            >
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300 justify-between h-[250px]">
            <div className="flex flex-col gap-2">
              <span className="text-[#B87932] font-mono font-bold text-xs uppercase tracking-wider">Panchayat</span>
              <h3 className="font-heading text-xl font-bold text-[#17352A]">Manage Village</h3>
              <p className="text-xs text-[#5F7668] leading-relaxed font-light">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300 justify-between h-[250px]">
            <div className="flex flex-col gap-2">
              <span className="text-[#B87932] font-mono font-bold text-xs uppercase tracking-wider">Collector</span>
              <h3 className="font-heading text-xl font-bold text-[#17352A]">District Dashboard</h3>
              <p className="text-xs text-[#5F7668] leading-relaxed font-light">
          ...
          <div className="bg-white border border-[#17352A]/10 p-6 rounded-2xl flex flex-col gap-4 shadow-sm hover:border-[#B87932]/30 transition-all hover:-translate-y-0.5 duration-300 justify-between h-[250px]">
            <div className="flex flex-col gap-2">
              <span className="text-[#B87932] font-mono font-bold text-xs uppercase tracking-wider">State Admin</span>
              <h3 className="font-heading text-xl font-bold text-[#17352A]">State Analytics</h3>
              <p className="text-xs text-[#5F7668] leading-relaxed font-light">
      ...
      {/* SECTION 11 — FOOTER */}
      <footer className="bg-[#17352A] border-t border-[#F5F1E7]/10 py-16 px-6 text-[#F5F1E7]">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12">
          
          <div className="md:col-span-5 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-white rounded-2xl p-1.5 shadow-md flex items-center justify-center shrink-0">
                <img src="/telangana-seal.png" alt="Government of Telangana Seal" className="w-full h-full object-contain" />
              </div>
              <div className="flex flex-col">
                <span className="font-heading font-extrabold text-xl text-white">GRAM Platform</span>
                <span className="text-[10px] text-[#B87932] font-mono uppercase tracking-wider">Government of Telangana • తెలంగాణ ప్రభుత్వం</span>
              </div>
            </div>
          ...
          <div className="bg-[#F5F1E7] border border-[#17352A]/15 w-full max-w-2xl rounded-3xl p-6 shadow-2xl relative flex flex-col max-h-[85vh] animate-fade-in-up">
            ...
            {modalSelectedDistrict && (
              <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-[#5F7668] bg-white border border-[#17352A]/5 px-3.5 py-2 rounded-xl font-mono mb-4">
                ...
                {modalSelectedMandal && (
                  <>
                    <span>/</span>
                    <span className="text-[#17352A]">{modalSelectedMandal.name}</span>
                  </>
                )}
              </div>
            )}
            ...
            {modalLoading ? (
              ...
              <div className="flex-grow overflow-y-auto custom-scroll pr-1">
                ...
                {/* Flow Step 0: Select District (if none selected yet) */}
                {!modalSelectedDistrict && (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {modalMandalsList.map((m) => (
                        <button
                          ...
                          className="text-left p-3.5 bg-white hover:bg-[#EAE3D4] border border-[#17352A]/10 rounded-xl text-xs font-semibold text-[#17352A] hover:text-[#B87932] transition-all flex justify-between items-center group shadow-sm"
                        >
                ...
                {modalSelectedMandal && !modalSelectedVillage && (
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <span className="text-xs text-[#17352A]/50 font-medium">Select Village ({modalVillagesList.length})</span>
                      <input
                        type="text"
                        placeholder="Search village..."
                        value={modalVillageSearch}
                        onChange={(e) => setModalVillageSearch(e.target.value)}
                        className="w-full sm:w-64 bg-white border border-[#17352A]/10 focus:border-[#B87932] rounded-xl px-3.5 py-2 text-xs text-[#17352A] outline-none shadow-sm"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-1 custom-scroll">
                      {modalVillagesList
                        ...
                        .map((v) => {
                          const riskColor = v.riskLevel === 'LOW' ? 'text-green-700 border-green-500/20 bg-green-500/10' :
                                            v.riskLevel === 'MEDIUM' ? 'text-yellow-700 border-yellow-500/20 bg-yellow-500/10' :
                                            'text-red-700 border-red-500/20 bg-red-500/10';
                          return (
                            <button
                              key={v.id}
                              onClick={() => {
                                setModalSelectedVillage(v);
                                setSelectedRole(null);
                                setIsLoginStep(false);
                              }}
                              className="text-left p-3.5 bg-white hover:bg-[#EAE3D4] border border-[#17352A]/10 rounded-xl text-xs font-semibold text-[#17352A] hover:text-[#B87932] transition-all flex justify-between items-center group shadow-sm"
                            >
                              <div className="flex flex-col gap-0.5 truncate">
                                ...
                                <span className="text-[8px] font-mono text-[#5F7668]">LGD Code: {v.id}</span>
                              </div>
                ...
                {/* Flow Step 3: Role Selection Screen */}
                {modalSelectedMandal && modalSelectedVillage && !selectedRole && (
                  <div className="flex flex-col gap-6 animate-fade-in py-2">
                    <div className="text-center flex flex-col gap-1.5 max-w-xl mx-auto mb-2">
                      <span className="text-[10px] font-mono font-bold text-[#B87932] uppercase tracking-widest">Welcome to your village</span>
                      <h2 className="font-heading font-black text-3xl text-[#17352A] tracking-wide">
                        {modalSelectedVillage.name.toUpperCase()}
                      </h2>
                      <p className="text-xs text-[#5F7668] font-mono">
                        {modalSelectedMandal.name} Mandal • {modalSelectedDistrict.name} District • Telangana State
                      </p>
                    </div>

                    <div className="border-t border-[#17352A]/10 pt-4 text-center">
                      <h3 className="text-sm font-heading font-bold text-[#17352A] mb-0.5">How are you connected to GRAM?</h3>
                      <p className="text-[10px] text-[#5F7668]/80">Choose your role to continue.</p>
                    </div>

                    {/* Role Cards Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-3.5 mt-2">
                      {[
                        { id: 'citizen', name: 'Citizen', desc: 'Explore your village and access public information.', icon: <User size={20} /> },
                        { id: 'panchayat', name: 'Village Secretary', desc: 'Manage local schemes, budgets and resolve grievances.', icon: <Award size={20} /> },
                        { id: 'tahsildar', name: 'Mandal Officer', desc: 'Approve resources and oversee panchayat records.', icon: <Building size={20} /> },
                        { id: 'collector', name: 'District Officer', desc: 'Review risk variables and direct regional operations.', icon: <Briefcase size={20} /> },
                        { id: 'state', name: 'State Officer', desc: 'State-wide statistics, audit compliance, and policy controls.', icon: <ShieldAlert size={20} /> }
                      ].map((role) => (
                        <div 
                          key={role.id}
                          className="bg-white hover:bg-[#EAE3D4] border border-[#17352A]/10 p-4 rounded-2xl flex flex-col justify-between items-center text-center h-[245px] transition-all duration-300 hover:-translate-y-1 hover:shadow-md group"
                        >
                          <div className="flex flex-col items-center gap-2.5">
                            <div className="w-10 h-10 bg-[#B87932]/10 rounded-xl flex items-center justify-center text-[#B87932]">
                              {role.icon}
                            </div>
                            <span className="text-[10px] font-mono font-bold text-[#17352A] uppercase tracking-wider">{role.name}</span>
                            <p className="text-[9px] text-[#5F7668] leading-relaxed font-light">
                              {role.desc}
                            </p>
                          </div>
                          <button
                            onClick={() => {
                              if (role.id === 'citizen') {
                                handleCloseModal();
                                navigate(`/public/village/${modalSelectedVillage.id}`);
                              } else {
                                setSelectedRole(role.id);
                                setIsLoginStep(true);
                              }
                            }}
                            className="w-full py-2 bg-[#F5F1E7] border border-[#17352A]/10 hover:bg-[#B87932] hover:text-white text-[#B87932] rounded-xl text-[9px] font-mono font-bold transition-all flex items-center justify-center gap-1 group-hover:scale-95 duration-300"
                          >
                            Access Path
                          </button>
                        </div>
                ...
                {/* Flow Step 4: Login Verification */}
                {modalSelectedMandal && modalSelectedVillage && selectedRole && isLoginStep && (
                  <div className="flex flex-col gap-6 max-w-md mx-auto animate-fade-in py-4">
                    <div className="text-center flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono font-bold text-[#B87932] uppercase tracking-widest">Authentication Gate</span>
                      <h2 className="font-heading font-black text-2xl text-[#17352A]">
                        {selectedRole === 'panchayat' ? 'Panchayat Secretary' :
                         selectedRole === 'tahsildar' ? 'Tahsildar / Mandal Officer' :
                         selectedRole === 'collector' ? 'Collector / District Officer' : 'State Officer'} Verification
                      </h2>
                      <p className="text-xs text-[#5F7668]">
                        Enter credentials authorized to manage LGD Scope {modalSelectedVillage.name}
                      </p>
                    </div>

                    <form onSubmit={handleModalLoginSubmit} className="flex flex-col gap-4 mt-2">
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] font-mono font-bold text-[#17352A] uppercase">Email Address</label>
                        <input
                          type="email"
                          required
                          ...
                          className="w-full bg-white border border-[#17352A]/10 focus:border-[#B87932] rounded-xl px-4 py-2.5 text-xs text-[#17352A] outline-none shadow-sm"
                        />
                      </div>

                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] font-mono font-bold text-[#17352A] uppercase">Password</label>
                        <input
                          type="password"
                          required
                          placeholder="••••••••"
                      ...
                      <div className="flex items-center justify-between mt-2 gap-4">
                        <button
                          ...
                          className="px-4 py-2.5 bg-white border border-[#17352A]/10 text-[#5F7668] hover:text-[#B87932] rounded-xl text-xs font-semibold transition-all shadow-sm"
                        >
                          Back to Roles
                        </button>
                        <button
                          ...
                          className="flex-grow py-2.5 bg-[#B87932] hover:bg-[#B87932]/90 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-2"
                        >
                          {loginLoading ? (
                            <>
